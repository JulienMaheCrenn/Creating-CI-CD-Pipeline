...
const viewAddUserModal = state => state.modal ? `
    <section id="modal" onclick="app.run('hideModal')">
        <form onclick="event.stopPropagation();" onsubmit="event.stopPropagation();app.run('addUser', this);return false;">
            <input name="name" placeholder="username" data-test-id="formName" required/>
            <input name="avatar" type="url" placeholder="avatar URL" data-test-id="formAvatar" required />
            <button data-test-id=formButton >Add User</button>
        </form>
    </section>
` : ""
